...
- (void)dismissPopupViewControllerWithanimationType:(MJPopupViewAnimation)animationType
{
    ...
    if(animationType == MJPopupViewAnimationSlideBottomTop || animationType == MJPopupViewAnimationSlideBottomBottom || animationType == MJPopupViewAnimationSlideRightLeft) {
        ...
        [self fadeViewOutSlidStop:popupView sourceView:sourceView overlayView:overlayView];
    }
}
...
- (void)presentPopupView:(UIView*)popupView animationType:(MJPopupViewAnimation)animationType
{
    ...
    if(animationType == MJPopupViewAnimationSlideBottomTop) {
        ...
        [dismissButton addTarget:self action:@selector(dismissPopupViewControllerWithanimationTypeFade) forControlEvents:UIControlEventTouchUpInside];
        [self fadeViewInSlid:popupView sourceView:sourceView overlayView:overlayView];
    }
}
...
- (void)fadeViewInSlid:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView
{
    UIView *backgroundView = [overlayView viewWithTag:kMJBackgroundViewTag];
    // Generating Start and Stop Positions
    CGSize sourceSize = sourceView.bounds.size;
    CGSize popupSize  = CGSizeMake(popupView.bounds.size.width*0.4,
                                   popupView.bounds.size.height*0.4);
    CGRect popupEndRect = CGRectMake((sourceSize.width - popupSize.width)/2,
                                     (sourceSize.height - popupSize.height)/2,
                                     popupSize.width,
                                     popupSize.height);
...
- (void)fadeViewOutSlidStop:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView
{
    CGRect rect = popupView.frame;
    UIView *backgroundView = [overlayView viewWithTag:kMJBackgroundViewTag];
    [UIView animateWithDuration:kPopupModalAnimationDuration animations:^{
        backgroundView.alpha = 0.0f;
        popupView.alpha = 0.0f;
        popupView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 0.4, 0.4);
        popupView.frame = [UIView fitCGRect:CGRectMake(0, rect.size.height/2, rect.size.width*0.4, rect.size.height*0.4)];
